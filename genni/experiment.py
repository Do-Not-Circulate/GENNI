...
if config["device"] == "gpu":
    tune.run(lambda config_inp: train(config_inp, folder_path), config=config, resources_per_trial={'gpu': 1}) # have a lambda that init's different center models. and then each one has some number of models that search. right
else:
