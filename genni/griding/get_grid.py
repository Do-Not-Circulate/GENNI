...
def main(experiment_folder, exp_id, config): 
    ...
    center_model = get_all_models(experiment_folder, config["center"]["step"])[config["center"]["idx"]][str(center_idx)]

    basis_vectors = [get_params_vec(get_all_models(experiment_folder, config["basis_vectors"]["steps"][i])[exp_id][str(config["basis_vectors"]["model_idxs"][i])]) 
    ...
    coordinates = [get_coordinates(b, basis_orthonorm_vectors, get_params_vec(center_model)) for b in basis_vectors]
    losses = [get_net_loss(vec_to_net(b, center_model), data_loader, criterion, full_dataset=True) for b in basis_vectors]

    # add center model     
    coordinates = [[0]*len(basis_vectors)] + coordinates
    losses = [get_net_loss(center_model, data_loader, criterion, full_dataset=True)] + losses 
    ...
    cache_data(os.path.join(experiment_folder, exp_id), "{}d_loss".format(len(basis_vectors)), three_d_vals, meta_dict=cache_dict, time_stamp=True)