...
def get_models_grid(offset, basis_vectors, num_inter_models, grid_bound):
    ...
    basis_m_dicts = [dict(vec_to_net(basis_vectors[i], offset).named_parameters()) for i in range(len(basis_vectors))]
    
    v_grid = []

    for coeffs in tqdm(itertools.product(np.linspace(grid_bound[0], grid_bound[1], num_inter_models), repeat=len(basis_vectors))):
        ...
        for name1, param1 in offset.named_parameters():
            v_to_add = sum([basis_m_dicts[i][name1].data * coeffs[-(i + 1)] for i in range(len(basis_vectors))])
...
def get_model_interpolate_grid(offset, basis_vectors, num_inter_models, grid_bound, func):
    
    basis_m_dicts = [dict(vec_to_net(basis_vectors[i], offset).named_parameters()) for i in range(len(basis_vectors))]
    
    val_arr = []

    for coeffs in tqdm(list(itertools.product(np.linspace(grid_bound[0], grid_bound[1], num_inter_models), repeat=len(basis_vectors)))):
        ...
        for name1, param1 in offset.named_parameters():
            v_to_add = sum([basis_m_dicts[i][name1].data * coeffs[-(i + 1)] for i in range(len(basis_vectors))]) # we are going in reverse because the first element in coeff corresponds to the last element of basis_vectors due to itertools.product
            dict_curr_model[name1].data.copy_(dict_curr_model[name1].data + v_to_add)
